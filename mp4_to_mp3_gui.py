...
class MP4ToMP3Extractor:
	...
	def setup_ui(self):
		...
		# Drag and drop area
		ttk.Label(main_frame, text='Drag and Drop MP4 File Here:').grid(
			row=0, column=0, sticky=tk.W, pady=5
		)

		drop_frame = tk.Frame(
			main_frame, bg='#e0e0e0', relief=tk.SUNKEN, borderwidth=2
		)
		drop_frame.grid(
			row=1,
			column=0,
		...
		ttk.Label(timeline_frame, text='End:').grid(
		...
		ttk.Label(quality_frame, text='Audio Quality:').pack(
		...
		self.status_label = ttk.Label(
			main_frame, text='Drop a file to begin', foreground='gray'
		)
	...
	def load_video_info(self, file_path):
		...
		if duration > 0:
			...
			self.video_info_label.config(
				text=f'Duration: {duration_str}', foreground='black'
			)
			self.status_label.config(
				text='Ready to extract', foreground='green'
			)
			self.extract_button.config(state=tk.NORMAL)
			self.update_timeline_labels()
		else:
			self.video_info_label.config(
				text='Could not detect video duration', foreground='red'
			)
			self.status_label.config(
				text='Error loading video', foreground='red'
			)
	...
	def update_timeline_labels(self, *args):
		...
		self.start_time_label.config(
			text=f'{self.format_time(start_time)} ({start_percent:.0f}%)'
		)
		self.end_time_label.config(
			text=f'{self.format_time(end_time)} ({end_percent:.0f}%)'
		)
	...
	def get_output_path(self):
		...
		output_name = f'{input_path.stem}_mp3.mp3'
	...
	def run_extraction(self):
		try:
			...
			if process.returncode == 0:
				self.root.after(
					0, lambda: self.extraction_complete(True, output_file)
				)
			else:
				error_msg = stderr if stderr else 'Unknown error'
				self.root.after(
					0,
					lambda: self.extraction_complete(
						False, output_file, error_msg
					),
				)
			...
			self.root.after(
				0, lambda: self.extraction_complete(False, None, str(e))
			)
	...
	def extraction_complete(self, success, output_file, error_msg=None):
		...
		if success:
			...
			messagebox.showinfo(
				'Success', f'Audio extracted successfully to:\n{output_file}'
			)
		else:
